...
To defend against advanced persistent threats (APTs) targeting the hospital's digital health terminals, we employ an Isolation Forest algorithm \cite{liu2008isolation}. The model isolates anomalies instead of profiling normal data points. The anomaly score $s(x, n)$ for a data point $x$ in a dataset of size $n$ is defined by its average path length $E(h(x))$:
\begin{equation}
s(x, n) = 2^{-\frac{E(h(x))}{c(n)}}
\end{equation}
where $c(n)$ is the average path length of unsuccessful searches in a Binary Search Tree. A score close to $1$ strongly indicates an anomaly.
...
The Isolation Forest model was evaluated on simulated telemetry traffic representing hospital API requests. Figure \ref{fig:ai} visualizes the isolation of malicious traffic clusters from normal baseline traffic.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{figures/ai_defense_anomalies.png}}
\caption{Isolation Forest anomaly detection on simulated network traffic.}
\label{fig:ai}
\end{figure}
