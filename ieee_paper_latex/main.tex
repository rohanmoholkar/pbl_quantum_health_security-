...
\subsection{AI Intrusion Detection System}
To empirically validate our defense against advanced persistent threats (APTs) targeting the hospital's digital health APIs, we employ a Random Forest ensemble learning algorithm trained on the standard KDD Cup 1999 cybersecurity dataset \cite{kddcup99}. The dataset was filtered to isolate numerical features representing normal traffic versus network attacks (e.g., DoS and Probing). The ensemble method utilizes a multitude of decision trees to output the mode of the classes for robust binary classification.
...
The Random Forest model was evaluated on 494,021 network traffic samples from the KDD Cup 99 dataset, achieving an empirical accuracy of 99.95\%. The confusion matrix (Figure \ref{fig:ai_cm}) demonstrates exceptional true positive rates for detecting malicious intrusions, while the ROC Curve (Figure \ref{fig:ai_roc}) confirms a near-perfect Area Under the Curve (AUC).

\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{figures/ai_confusion_matrix.png}}
\caption{Confusion Matrix for AI Intrusion Detection.}
\label{fig:ai_cm}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{figures/ai_roc_curve.png}}
